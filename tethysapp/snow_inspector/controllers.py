...
def snow_graph(request):
    ...
    if request.GET:
        ...
        map_url = "?days=%s&end=%s&lat=%s&lon=%s&zoom=%s" % (
            numdays,
            endDate,
            lat,
            lon,
...
def upload_to_hydroshare(request):
    ...
    try:
        ...
        print((str(e)))
        return_json["error"] = (
            'Object doesn"t exist: Login timed out! Please re-sign in with your HydroShare account.'
        )
    except TokenExpiredError as e:
        print((str(e)))
        return_json["error"] = (
            "Login timed out! Please re-sign in with your HydroShare account."
        )
        ...
        if "401 Unauthorized" in str(err):
            ...
            return_json["error"] = (
                "File uploaded successfully despite 400 Bad Request Error."
            )
        else:
            traceback.print_exc()
